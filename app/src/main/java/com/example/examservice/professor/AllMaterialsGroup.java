...
public class AllMaterialsGroup extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.professor_all_materials_group);

        Intent intent = getIntent();
        showFiles = intent.getBooleanExtra("SHOW_FILES", false);
        addGroupExam = intent.getBooleanExtra("ADD_MATERIAL_TO_EXAM", false);

        allGroupsList = MainProfessorActivity.groupsList;
        groupsList = new ArrayList<>();
        groupExam = new ArrayList<>();

        if(showFiles){
            groupExam = ExamOptions.groupExamList;
            getList();
        }else{
            groupsList = allGroupsList ;
        }

        recyclerView = findViewById(R.id.allProfMaterialGroupsListView);
    ...
    public void onMaterialGroupClick(View view) {
        ...
        if(addGroupExam){
            Exam exam = AllExamsList.currentExam;
            int counter = ExamOptions.qroupExamCount ;
            counter++;
